...
## 如果你打算自己跟着教程实现多人插件，那我真心建议你使用和作者相同的虚幻5.0版本，如果你要用我的源码那我建议你也使用我的虚幻5.4版本，因为这个不同版本很容易出Bug，如果只是用来学习虚幻，我真心建议不必要为了这个插件浪费过多时间，这个教程太老了，所以我觉得更有价值的是作者制作的各种多人联机网络复制和预测方面的功能实现，为了完全复现这个插件本身的意义我觉得并不大，如果你就是为了去学习这种插件就要搞懂这类插件可能会遇到的问题，那就尽情去Debug吧。
...
*那么这条多播RPC其实可以优化，没必要再去发送到其余的客户端造成无意义的网络资源耗费。
*所以我修改多播RPC为指定客户端RPC，在装备时服务器只会向玩家控制的这台客户端进行同步纠正弹药。
*但是注意这一步需要在设置完武器的所有者之后执行，也就是需要先确认武器的所有客户端，否则服务器无法确认该向哪个客户端发送RPC。
*修改CombatComponent.cpp的代码如下。删除装备武器方法当中的多播RPC调用。在装备主副武器的方法中添加指定客户端RPC的调用。
*/
...
//其实实际是因为，作者在设计的时候，倒带判断伤害时，先计算的头部HitBox，把其他部位的HitBox设置无碰撞，这就导致其实实际上的判定是穿过了外部的HitBox直接打中头部了。
